...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Service.query(function(qb) {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Service._attributes)
      .filter(attribute => attribute !== Service.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Service._attributes[attribute].type));
